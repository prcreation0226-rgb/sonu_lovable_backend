// Radiantilyk EMR — Client Service Catalog Service
// Contains complete service catalog definition and idempotent MySQL import logic.
...
export const CLIENT_SERVICE_CATALOG: CategoryData[] = [
  {
    name: 'Consultations',
...
export async function importServiceCatalog() {
  ...
  for (const catData of CLIENT_SERVICE_CATALOG) {
  ...
  console.log(`- Categories Processed: ${CLIENT_SERVICE_CATALOG.length} (${totalCategoriesCreated} created, ${totalCategoriesUpdated} updated)`);
  console.log(`- Services Processed: ${CLIENT_SERVICE_CATALOG.reduce((sum, c) => sum + c.services.length, 0)} (${totalServicesCreated} created, ${totalServicesUpdated} updated)`);
  console.log('----------------------------------------------------------------\n');
}
