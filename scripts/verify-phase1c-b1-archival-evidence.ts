...
async function makeRequest(
  ...
  return new Promise((resolve, reject) => {
    ...
    const reqHeaders: Record<string, string> = {
      ...
      'User-Agent': 'Phase1C-B1-Evidence-Suite/1.0',
      ...customHeaders,
    };
...
async function runEvidenceAudit() {
  ...
  const npLogin = await makeRequest(
    'POST',
    '/auth/login',
    { email: 'phase1-np@radiantilyk.com', password: 'Phase1Test!2026' },
    undefined,
    { 'X-Test-MFA-Enforcement': 'true' }
  );

  const hasMfaPendingCookie = npLogin.cookies.some((c) => c.includes('rka_mfa_pending'));
  const hasAccessCookie = npLogin.cookies.some((c) => c.includes('rka_access'));
  const hasRefreshCookie = npLogin.cookies.some((c) => c.includes('rka_refresh'));
  const enrollmentRequired = npLogin.body.data?.enrollmentRequired === true;

  // Complete enrollment via pending cookie
  const startEnroll = await makeRequest('POST', '/auth/mfa/enroll/start', undefined, npLogin.cookies);
  ...
  results.push({
    ...
    actualStatus: `Login: ${npLogin.status}, Verify: ${verifyEnroll.status}`,
  ...
  // Attempt to access an MFA-protected endpoint (e.g. regenerate recovery codes)
  ...
  results.push({
    ...
    evidenceDetails: `Session with 15-minute MFA age rejected with 403 Forbidden. Message: ${expiredSessionRes.body.error?.message || expiredSessionRes.body.message}`,
  });

  // ----------------------------------------------------------------
  // Evidence 5: Origin / CSRF Protection
  // ----------------------------------------------------------------
  console.log('Executing Evidence 5: Origin/CSRF protection for state-changing endpoints...');
  ...
  // Print Summary Table
  console.log('\n----------------------------------------------------------------');
  console.log('ARCHIVAL EVIDENCE AUDIT RESULTS:');
  console.log('----------------------------------------------------------------');
...
runEvidenceAudit().catch((err) => {
  console.error('Error running evidence audit:', err);
  process.exit(1);
});
